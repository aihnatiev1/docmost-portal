...
import { DocsPortalContext } from "../context/docs-portal-context";
...
export default function DocsPortalLayout() {
  const { locale, spaceSlug, pageSlug } = useParams<{
    locale?: string;
    spaceSlug: string;
    pageSlug?: string;
  }>();
  const basePath = locale ? `/${locale}/docs` : '/docs';
  ...
  useEffect(() => {
    if (!pageSlug && homePage) {
      const slug = homePage.title
        ...
        : homePage.slugId;
      navigate(`${basePath}/${spaceSlug}/${slug}`, { replace: true });
    }
  ...
  return (
    <DocsPortalContext.Provider value={{ basePath, locale }}>
      <Helmet>
        <title>
          {pageData?.page?.title
