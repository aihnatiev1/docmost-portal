...
export default function DocsFooter({
  ...
  const { basePath } = useDocsPortalContext();
    ...
    <div>
      ...
      {showPagination && (prevPage || nextPage) && (
        <div className={classes.pageNavigation}>
          {prevPage ? (
            <Link
              to={`${basePath}/${spaceSlug}/${buildPageSlug(prevPage.title, prevPage.slugId)}`}
          ...
          {nextPage ? (
            <Link
              to={`${basePath}/${spaceSlug}/${buildPageSlug(nextPage.title, nextPage.slugId)}`}
